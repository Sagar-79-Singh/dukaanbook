...
module.exports = async (req, res) => {
  ...
  if (req.query.key !== 'dukaanbook_init_2024') {
  ...
  try {
    ...
    // Simple units
    for (const u of ['Pcs','Box','Dozen','Kg','Gram','Litre','Packet','Ream','Bundle','Roll','Metre']) {
    ...
    for (const [name, base, qty] of compounds) {
      const [[baseUnit]] = await pool.query(`SELECT id FROM units WHERE name=?`, [base]);
      if (baseUnit) {
        await pool.query(`INSERT IGNORE INTO units (name, base_unit_id, conversion_qty) VALUES (?,?,?)`,
          [name, baseUnit.id, qty]);
      }
    }

    // Groups
    for (const g of ['Pen & Pencil','Paper & Notebook','File & Folder','Ink & Refill','Stationery Misc','Computer Accessories','Art & Craft','Rubber & Eraser']) {
    ...
    await pool.query(`INSERT INTO users (username,password_hash,role) VALUES (?,?,'admin') ON DUPLICATE KEY UPDATE password_hash=?`, ['admin', adminHash, adminHash]);
    await pool.query(`INSERT INTO users (username,password_hash,role) VALUES (?,?,'user') ON DUPLICATE KEY UPDATE password_hash=?`, ['dad', dadHash, dadHash]);

    res.json({
      success: true,
      message: 'Database ready! Sab tables ban gaye.',
      users: [
